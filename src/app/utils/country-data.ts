export const countryArrayEN = [
    {
        "name": "Afghanistan",
        "code2": "AF",
        "code3": "AFG"
    },
    {
        "name": "Albania",
        "code2": "AL",
        "code3": "ALB"
    },
    {
        "name": "Algeria",
        "code2": "DZ",
        "code3": "DZA"
    },
    {
        "name": "Andorra",
        "code2": "AD",
        "code3": "AND"
    },
    {
        "name": "Angola",
        "code2": "AO",
        "code3": "AGO"
    },
    {
        "name": "Antigua and Barbuda",
        "code2": "AG",
        "code3": "ATG"
    },
    {
        "name": "Argentina",
        "code2": "AR",
        "code3": "ARG"
    },
    {
        "name": "Armenia",
        "code2": "AM",
        "code3": "ARM"
    },
    {
        "name": "Australia",
        "code2": "AU",
        "code3": "AUS"
    },
    {
        "name": "Austria",
        "code2": "AT",
        "code3": "AUT"
    },
    {
        "name": "Azerbaijan",
        "code2": "AZ",
        "code3": "AZE"
    },
    {
        "name": "Bahamas",
        "code2": "BS",
        "code3": "BHS"
    },
    {
        "name": "Bahrain",
        "code2": "BH",
        "code3": "BHR"
    },
    {
        "name": "Bangladesh",
        "code2": "BD",
        "code3": "BGD"
    },
    {
        "name": "Barbados",
        "code2": "BB",
        "code3": "BRB"
    },
    {
        "name": "Belarus",
        "code2": "BY",
        "code3": "BLR"
    },
    {
        "name": "Belgium",
        "code2": "BE",
        "code3": "BEL"
    },
    {
        "name": "Belize",
        "code2": "BZ",
        "code3": "BLZ"
    },
    {
        "name": "Benin",
        "code2": "BJ",
        "code3": "BEN"
    },
    {
        "name": "Bhutan",
        "code2": "BT",
        "code3": "BTN"
    },
    {
        "name": "Bolivia",
        "code2": "BO",
        "code3": "BOL"
    },
    {
        "name": "Bosnia and Herzegovina",
        "code2": "BA",
        "code3": "BIH"
    },
    {
        "name": "Botswana",
        "code2": "BW",
        "code3": "BWA"
    },
    {
        "name": "Brazil",
        "code2": "BR",
        "code3": "BRA"
    },
    {
        "name": "Brunei",
        "code2": "BN",
        "code3": "BRN"
    },
    {
        "name": "Bulgaria",
        "code2": "BG",
        "code3": "BGR"
    },
    {
        "name": "Burkina Faso",
        "code2": "BF",
        "code3": "BFA"
    },
    {
        "name": "Burundi",
        "code2": "BI",
        "code3": "BDI"
    },
    {
        "name": "Cape Verde",
        "code2": "CV",
        "code3": "CPV"
    },
    {
        "name": "Cambodia",
        "code2": "KH",
        "code3": "KHM"
    },
    {
        "name": "Cameroon",
        "code2": "CM",
        "code3": "CMR"
    },
    {
        "name": "Canada",
        "code2": "CA",
        "code3": "CAN"
    },
    {
        "name": "Central African Republic",
        "code2": "CF",
        "code3": "CAF"
    },
    {
        "name": "Chad",
        "code2": "TD",
        "code3": "TCD"
    },
    {
        "name": "Chile",
        "code2": "CL",
        "code3": "CHL"
    },
    {
        "name": "China",
        "code2": "CN",
        "code3": "CHN"
    },
    {
        "name": "Colombia",
        "code2": "CO",
        "code3": "COL"
    },
    {
        "name": "Comoros",
        "code2": "KM",
        "code3": "COM"
    },
    {
        "name": "Democratic Republic of the Congo",
        "code2": "CD",
        "code3": "COD"
    },
    {
        "name": "Congo",
        "code2": "CG",
        "code3": "COG"
    },
    {
        "name": "Costa Rica",
        "code2": "CR",
        "code3": "CRI"
    },
    {
        "name": "Cote d'Ivoire (Ivory Coast)",
        "code2": "CI",
        "code3": "CIV"
    },
    {
        "name": "Croatia",
        "code2": "HR",
        "code3": "HRV"
    },
    {
        "name": "Cuba",
        "code2": "CU",
        "code3": "CUB"
    },
    {
        "name": "Cyprus",
        "code2": "CY",
        "code3": "CYP"
    },
    {
        "name": "Czech Republic",
        "code2": "CZ",
        "code3": "CZE"
    },
    {
        "name": "Denmark",
        "code2": "DK",
        "code3": "DNK"
    },
    {
        "name": "Djibouti",
        "code2": "DJ",
        "code3": "DJI"
    },
    {
        "name": "Dominica",
        "code2": "DM",
        "code3": "DMA"
    },
    {
        "name": "Dominican Republic",
        "code2": "DO",
        "code3": "DOM"
    },
    {
        "name": "Ecuador",
        "code2": "EC",
        "code3": "ECU"
    },
    {
        "name": "Egypt",
        "code2": "EG",
        "code3": "EGY"
    },
    {
        "name": "El Salvador",
        "code2": "SV",
        "code3": "SLV"
    },
    {
        "name": "Equatorial Guinea",
        "code2": "GQ",
        "code3": "GNQ"
    },
    {
        "name": "Eritrea",
        "code2": "ER",
        "code3": "ERI"
    },
    {
        "name": "Estonia",
        "code2": "EE",
        "code3": "EST"
    },
    {
        "name": "Eswatini",
        "code2": "SZ",
        "code3": "SWZ"
    },
    {
        "name": "Ethiopia",
        "code2": "ET",
        "code3": "ETH"
    },
    {
        "name": "Fiji Islands",
        "code2": "FJ",
        "code3": "FJI"
    },
    {
        "name": "Finland",
        "code2": "FI",
        "code3": "FIN"
    },
    {
        "name": "France",
        "code2": "FR",
        "code3": "FRA"
    },
    {
        "name": "Gabon",
        "code2": "GA",
        "code3": "GAB"
    },
    {
        "name": "Gambia The",
        "code2": "GM",
        "code3": "GMB"
    },
    {
        "name": "Georgia",
        "code2": "GE",
        "code3": "GEO"
    },
    {
        "name": "Germany",
        "code2": "DE",
        "code3": "DEU"
    },
    {
        "name": "Ghana",
        "code2": "GH",
        "code3": "GHA"
    },
    {
        "name": "Greece",
        "code2": "GR",
        "code3": "GRC"
    },
    {
        "name": "Grenada",
        "code2": "GD",
        "code3": "GRD"
    },
    {
        "name": "Guatemala",
        "code2": "GT",
        "code3": "GTM"
    },
    {
        "name": "Guinea",
        "code2": "GN",
        "code3": "GIN"
    },
    {
        "name": "Guinea-Bissau",
        "code2": "GW",
        "code3": "GNB"
    },
    {
        "name": "Guyana",
        "code2": "GY",
        "code3": "GUY"
    },
    {
        "name": "Haiti",
        "code2": "HT",
        "code3": "HTI"
    },
    {
        "name": "Honduras",
        "code2": "HN",
        "code3": "HND"
    },
    {
        "name": "Hungary",
        "code2": "HU",
        "code3": "HUN"
    },
    {
        "name": "Iceland",
        "code2": "IS",
        "code3": "ISL"
    },
    {
        "name": "India",
        "code2": "IN",
        "code3": "IND"
    },
    {
        "name": "Indonesia",
        "code2": "ID",
        "code3": "IDN"
    },
    {
        "name": "Iran",
        "code2": "IR",
        "code3": "IRN"
    },
    {
        "name": "Iraq",
        "code2": "IQ",
        "code3": "IRQ"
    },
    {
        "name": "Ireland",
        "code2": "IE",
        "code3": "IRL"
    },
    {
        "name": "Israel",
        "code2": "IL",
        "code3": "ISR"
    },
    {
        "name": "Italy",
        "code2": "IT",
        "code3": "ITA"
    },
    {
        "name": "Jamaica",
        "code2": "JM",
        "code3": "JAM"
    },
    {
        "name": "Japan",
        "code2": "JP",
        "code3": "JPN"
    },
    {
        "name": "Jordan",
        "code2": "JO",
        "code3": "JOR"
    },
    {
        "name": "Kazakhstan",
        "code2": "KZ",
        "code3": "KAZ"
    },
    {
        "name": "Kenya",
        "code2": "KE",
        "code3": "KEN"
    },
    {
        "name": "Kiribati",
        "code2": "KI",
        "code3": "KIR"
    },
    {
        "name": "North Korea",
        "code2": "KP",
        "code3": "PRK"
    },
    {
        "name": "South Korea",
        "code2": "KR",
        "code3": "KOR"
    },
    {
        "name": "Kuwait",
        "code2": "KW",
        "code3": "KWT"
    },
    {
        "name": "Kyrgyzstan",
        "code2": "KG",
        "code3": "KGZ"
    },
    {
        "name": "Laos",
        "code2": "LA",
        "code3": "LAO"
    },
    {
        "name": "Latvia",
        "code2": "LV",
        "code3": "LVA"
    },
    {
        "name": "Lebanon",
        "code2": "LB",
        "code3": "LBN"
    },
    {
        "name": "Lesotho",
        "code2": "LS",
        "code3": "LSO"
    },
    {
        "name": "Liberia",
        "code2": "LR",
        "code3": "LBR"
    },
    {
        "name": "Libya",
        "code2": "LY",
        "code3": "LBY"
    },
    {
        "name": "Liechtenstein",
        "code2": "LI",
        "code3": "LIE"
    },
    {
        "name": "Lithuania",
        "code2": "LT",
        "code3": "LTU"
    },
    {
        "name": "Luxembourg",
        "code2": "LU",
        "code3": "LUX"
    },
    {
        "name": "Madagascar",
        "code2": "MG",
        "code3": "MDG"
    },
    {
        "name": "Malawi",
        "code2": "MW",
        "code3": "MWI"
    },
    {
        "name": "Malaysia",
        "code2": "MY",
        "code3": "MYS"
    },
    {
        "name": "Maldives",
        "code2": "MV",
        "code3": "MDV"
    },
    {
        "name": "Mali",
        "code2": "ML",
        "code3": "MLI"
    },
    {
        "name": "Malta",
        "code2": "MT",
        "code3": "MLT"
    },
    {
        "name": "Marshall Islands",
        "code2": "MH",
        "code3": "MHL"
    },
    {
        "name": "Mauritania",
        "code2": "MR",
        "code3": "MRT"
    },
    {
        "name": "Mauritius",
        "code2": "MU",
        "code3": "MUS"
    },
    {
        "name": "Mexico",
        "code2": "MX",
        "code3": "MEX"
    },
    {
        "name": "Micronesia",
        "code2": "FM",
        "code3": "FSM"
    },
    {
        "name": "Moldova",
        "code2": "MD",
        "code3": "MDA"
    },
    {
        "name": "Monaco",
        "code2": "MC",
        "code3": "MCO"
    },
    {
        "name": "Mongolia",
        "code2": "MN",
        "code3": "MNG"
    },
    {
        "name": "Montenegro",
        "code2": "ME",
        "code3": "MNE"
    },
    {
        "name": "Morocco",
        "code2": "MA",
        "code3": "MAR"
    },
    {
        "name": "Mozambique",
        "code2": "MZ",
        "code3": "MOZ"
    },
    {
        "name": "Myanmar",
        "code2": "MM",
        "code3": "MMR"
    },
    {
        "name": "Namibia",
        "code2": "NA",
        "code3": "NAM"
    },
    {
        "name": "Nauru",
        "code2": "NR",
        "code3": "NRU"
    },
    {
        "name": "Nepal",
        "code2": "NP",
        "code3": "NPL"
    },
    {
        "name": "Netherlands",
        "code2": "NL",
        "code3": "NLD"
    },
    {
        "name": "New Zealand",
        "code2": "NZ",
        "code3": "NZL"
    },
    {
        "name": "Nicaragua",
        "code2": "NI",
        "code3": "NIC"
    },
    {
        "name": "Niger",
        "code2": "NE",
        "code3": "NER"
    },
    {
        "name": "Nigeria",
        "code2": "NG",
        "code3": "NGA"
    },
    {
        "name": "North Macedonia",
        "code2": "MK",
        "code3": "MKD"
    },
    {
        "name": "Norway",
        "code2": "NO",
        "code3": "NOR"
    },
    {
        "name": "Oman",
        "code2": "OM",
        "code3": "OMN"
    },
    {
        "name": "Pakistan",
        "code2": "PK",
        "code3": "PAK"
    },
    {
        "name": "Palau",
        "code2": "PW",
        "code3": "PLW"
    },
    {
        "name": "Panama",
        "code2": "PA",
        "code3": "PAN"
    },
    {
        "name": "Papua New Guinea",
        "code2": "PG",
        "code3": "PNG"
    },
    {
        "name": "Paraguay",
        "code2": "PY",
        "code3": "PRY"
    },
    {
        "name": "Peru",
        "code2": "PE",
        "code3": "PER"
    },
    {
        "name": "Philippines",
        "code2": "PH",
        "code3": "PHL"
    },
    {
        "name": "Poland",
        "code2": "PL",
        "code3": "POL"
    },
    {
        "name": "Portugal",
        "code2": "PT",
        "code3": "PRT"
    },
    {
        "name": "Qatar",
        "code2": "QA",
        "code3": "QAT"
    },
    {
        "name": "Romania",
        "code2": "RO",
        "code3": "ROU"
    },
    {
        "name": "Russia",
        "code2": "RU",
        "code3": "RUS"
    },
    {
        "name": "Rwanda",
        "code2": "RW",
        "code3": "RWA"
    },
    {
        "name": "Saint Kitts and Nevis",
        "code2": "KN",
        "code3": "KNA"
    },
    {
        "name": "Saint Lucia",
        "code2": "LC",
        "code3": "LCA"
    },
    {
        "name": "Saint Vincent and the Grenadines",
        "code2": "VC",
        "code3": "VCT"
    },
    {
        "name": "Samoa",
        "code2": "WS",
        "code3": "WSM"
    },
    {
        "name": "San Marino",
        "code2": "SM",
        "code3": "SMR"
    },
    {
        "name": "Sao Tome and Principe",
        "code2": "ST",
        "code3": "STP"
    },
    {
        "name": "Saudi Arabia",
        "code2": "SA",
        "code3": "SAU"
    },
    {
        "name": "Senegal",
        "code2": "SN",
        "code3": "SEN"
    },
    {
        "name": "Serbia",
        "code2": "RS",
        "code3": "SRB"
    },
    {
        "name": "Seychelles",
        "code2": "SC",
        "code3": "SYC"
    },
    {
        "name": "Sierra Leone",
        "code2": "SL",
        "code3": "SLE"
    },
    {
        "name": "Singapore",
        "code2": "SG",
        "code3": "SGP"
    },
    {
        "name": "Slovakia",
        "code2": "SK",
        "code3": "SVK"
    },
    {
        "name": "Slovenia",
        "code2": "SI",
        "code3": "SVN"
    },
    {
        "name": "Solomon Islands",
        "code2": "SB",
        "code3": "SLB"
    },
    {
        "name": "Somalia",
        "code2": "SO",
        "code3": "SOM"
    },
    {
        "name": "South Africa",
        "code2": "ZA",
        "code3": "ZAF"
    },
    {
        "name": "South Sudan",
        "code2": "SS",
        "code3": "SSD"
    },
    {
        "name": "Spain",
        "code2": "ES",
        "code3": "ESP"
    },
    {
        "name": "Sri Lanka",
        "code2": "LK",
        "code3": "LKA"
    },
    {
        "name": "Sudan",
        "code2": "SD",
        "code3": "SDN"
    },
    {
        "name": "Suriname",
        "code2": "SR",
        "code3": "SUR"
    },
    {
        "name": "Sweden",
        "code2": "SE",
        "code3": "SWE"
    },
    {
        "name": "Switzerland",
        "code2": "CH",
        "code3": "CHE"
    },
    {
        "name": "Syria",
        "code2": "SY",
        "code3": "SYR"
    },
    {
        "name": "Tajikistan",
        "code2": "TJ",
        "code3": "TJK"
    },
    {
        "name": "Tanzania",
        "code2": "TZ",
        "code3": "TZA"
    },
    {
        "name": "Thailand",
        "code2": "TH",
        "code3": "THA"
    },
    {
        "name": "East Timor",
        "code2": "TL",
        "code3": "TLS"
    },
    {
        "name": "Togo",
        "code2": "TG",
        "code3": "TGO"
    },
    {
        "name": "Tonga",
        "code2": "TO",
        "code3": "TON"
    },
    {
        "name": "Trinidad and Tobago",
        "code2": "TT",
        "code3": "TTO"
    },
    {
        "name": "Tunisia",
        "code2": "TN",
        "code3": "TUN"
    },
    {
        "name": "Turkey",
        "code2": "TR",
        "code3": "TUR"
    },
    {
        "name": "Turkmenistan",
        "code2": "TM",
        "code3": "TKM"
    },
    {
        "name": "Tuvalu",
        "code2": "TV",
        "code3": "TUV"
    },
    {
        "name": "Uganda",
        "code2": "UG",
        "code3": "UGA"
    },
    {
        "name": "Ukraine",
        "code2": "UA",
        "code3": "UKR"
    },
    {
        "name": "United Arab Emirates",
        "code2": "AE",
        "code3": "ARE"
    },
    {
        "name": "United Kingdom",
        "code2": "GB",
        "code3": "GBR"
    },
    {
        "name": "United States",
        "code2": "US",
        "code3": "USA"
    },
    {
        "name": "Uruguay",
        "code2": "UY",
        "code3": "URY"
    },
    {
        "name": "Uzbekistan",
        "code2": "UZ",
        "code3": "UZB"
    },
    {
        "name": "Vanuatu",
        "code2": "VU",
        "code3": "VUT"
    },
    {
        "name": "Venezuela",
        "code2": "VE",
        "code3": "VEN"
    },
    {
        "name": "Vietnam",
        "code2": "VN",
        "code3": "VNM"
    },
    {
        "name": "Yemen",
        "code2": "YE",
        "code3": "YEM"
    },
    {
        "name": "Zambia",
        "code2": "ZM",
        "code3": "ZMB"
    },
    {
        "name": "Zimbabwe",
        "code2": "ZW",
        "code3": "ZWE"
    }
]
export const countryArrayFR = [
    {
        "code2": "AF",
        "name_fr": "Afghanistan"
    },
    {
        "code2": "ZA",
        "name_fr": "Afrique du Sud"
    },
    {
        "code2": "AL",
        "name_fr": "Albanie"
    },
    {
        "code2": "DZ",
        "name_fr": "Algérie"
    },
    {
        "code2": "DE",
        "name_fr": "Allemagne"
    },
    {
        "code2": "AD",
        "name_fr": "Andorre"
    },
    {
        "code2": "AO",
        "name_fr": "Angola"
    },
    {
        "code2": "AG",
        "name_fr": "Antigua-et-Barbuda"
    },
    {
        "code2": "SA",
        "name_fr": "Arabie saoudite"
    },
    {
        "code2": "AR",
        "name_fr": "Argentine"
    },
    {
        "code2": "AM",
        "name_fr": "Arménie"
    },
    {
        "code2": "AU",
        "name_fr": "Australie"
    },
    {
        "code2": "AT",
        "name_fr": "Autriche"
    },
    {
        "code2": "AZ",
        "name_fr": "Azerbaïdjan"
    },
    {
        "code2": "BS",
        "name_fr": "Bahamas"
    },
    {
        "code2": "BH",
        "name_fr": "Bahreïn"
    },
    {
        "code2": "BD",
        "name_fr": "Bangladesh"
    },
    {
        "code2": "BB",
        "name_fr": "Barbade"
    },
    {
        "code2": "BY",
        "name_fr": "Biélorussie"
    },
    {
        "code2": "BE",
        "name_fr": "Belgique"
    },
    {
        "code2": "BZ",
        "name_fr": "Belize"
    },
    {
        "code2": "BJ",
        "name_fr": "Bénin"
    },
    {
        "code2": "BT",
        "name_fr": "Bhoutan"
    },
    {
        "code2": "BO",
        "name_fr": "Bolivie"
    },
    {
        "code2": "BA",
        "name_fr": "Bosnie-Herzégovine"
    },
    {
        "code2": "BW",
        "name_fr": "Botswana"
    },
    {
        "code2": "BR",
        "name_fr": "Brésil"
    },
    {
        "code2": "BN",
        "name_fr": "Brunei"
    },
    {
        "code2": "BG",
        "name_fr": "Bulgarie"
    },
    {
        "code2": "BF",
        "name_fr": "Burkina Faso"
    },
    {
        "code2": "BI",
        "name_fr": "Burundi"
    },
    {
        "code2": "KH",
        "name_fr": "Cambodge"
    },
    {
        "code2": "CM",
        "name_fr": "Cameroun"
    },
    {
        "code2": "CA",
        "name_fr": "Canada"
    },
    {
        "code2": "CV",
        "name_fr": "Cap-Vert"
    },
    {
        "code2": "CF",
        "name_fr": "République centrafricaine"
    },
    {
        "code2": "CL",
        "name_fr": "Chili"
    },
    {
        "code2": "CN",
        "name_fr": "Chine"
    },
    {
        "code2": "CY",
        "name_fr": "Chypre"
    },
    {
        "code2": "CO",
        "name_fr": "Colombie"
    },
    {
        "code2": "KM",
        "name_fr": "Comores"
    },
    {
        "code2": "CG",
        "name_fr": "République du Congo"
    },
    {
        "code2": "CD",
        "name_fr": "République démocratique du Congo"
    },
    {
        "code2": "KR",
        "name_fr": "Corée du Sud"
    },
    {
        "code2": "KP",
        "name_fr": "Corée du Nord"
    },
    {
        "code2": "CR",
        "name_fr": "Costa Rica"
    },
    {
        "code2": "CI",
        "name_fr": "Côte d'Ivoire"
    },
    {
        "code2": "HR",
        "name_fr": "Croatie"
    },
    {
        "code2": "CU",
        "name_fr": "Cuba"
    },
    {
        "code2": "DK",
        "name_fr": "Danemark"
    },
    {
        "code2": "DJ",
        "name_fr": "Djibouti"
    },
    {
        "code2": "DO",
        "name_fr": "République dominicaine"
    },
    {
        "code2": "DM",
        "name_fr": "Dominique"
    },
    {
        "code2": "EG",
        "name_fr": "Égypte"
    },
    {
        "code2": "AE",
        "name_fr": "Émirats arabes unis"
    },
    {
        "code2": "EC",
        "name_fr": "Équateur"
    },
    {
        "code2": "ER",
        "name_fr": "Érythrée"
    },
    {
        "code2": "ES",
        "name_fr": "Espagne"
    },
    {
        "code2": "EE",
        "name_fr": "Estonie"
    },
    {
        "code2": "US",
        "name_fr": "États-Unis"
    },
    {
        "code2": "ET",
        "name_fr": "Éthiopie"
    },
    {
        "code2": "FJ",
        "name_fr": "Iles Fiji"
    },
    {
        "code2": "FI",
        "name_fr": "Finlande"
    },
    {
        "code2": "FR",
        "name_fr": "France"
    },
    {
        "code2": "GA",
        "name_fr": "Gabon"
    },
    {
        "code2": "GM",
        "name_fr": "Gambie"
    },
    {
        "code2": "GE",
        "name_fr": "Géorgie"
    },
    {
        "code2": "GH",
        "name_fr": "Ghana"
    },
    {
        "code2": "GR",
        "name_fr": "Grèce"
    },
    {
        "code2": "GD",
        "name_fr": "Grenade"
    },
    {
        "code2": "GT",
        "name_fr": "Guatemala"
    },
    {
        "code2": "GN",
        "name_fr": "Guinée"
    },
    {
        "code2": "GW",
        "name_fr": "Guinée-Bissau"
    },
    {
        "code2": "GQ",
        "name_fr": "Guinée équatoriale"
    },
    {
        "code2": "GY",
        "name_fr": "Guyana"
    },
    {
        "code2": "HT",
        "name_fr": "Haïti"
    },
    {
        "code2": "HN",
        "name_fr": "Honduras"
    },
    {
        "code2": "HU",
        "name_fr": "Hongrie"
    },
    {
        "code2": "IN",
        "name_fr": "Inde"
    },
    {
        "code2": "ID",
        "name_fr": "Indonésie"
    },
    {
        "code2": "IR",
        "name_fr": "Iran"
    },
    {
        "code2": "IQ",
        "name_fr": "Irak"
    },
    {
        "code2": "IE",
        "name_fr": "Irlande"
    },
    {
        "code2": "IS",
        "name_fr": "Islande"
    },
    {
        "code2": "IL",
        "name_fr": "Israël"
    },
    {
        "code2": "IT",
        "name_fr": "Italie"
    },
    {
        "code2": "JM",
        "name_fr": "Jamaïque"
    },
    {
        "code2": "JP",
        "name_fr": "Japon"
    },
    {
        "code2": "JO",
        "name_fr": "Jordanie"
    },
    {
        "code2": "KZ",
        "name_fr": "Kazakhstan"
    },
    {
        "code2": "KE",
        "name_fr": "Kenya"
    },
    {
        "code2": "KG",
        "name_fr": "Kirghizistan"
    },
    {
        "code2": "KI",
        "name_fr": "Kiribati"
    },
    {
        "code2": "KW",
        "name_fr": "Koweït"
    },
    {
        "code2": "LA",
        "name_fr": "Laos"
    },
    {
        "code2": "LS",
        "name_fr": "Lesotho"
    },
    {
        "code2": "LV",
        "name_fr": "Lettonie"
    },
    {
        "code2": "LB",
        "name_fr": "Liban"
    },
    {
        "code2": "LR",
        "name_fr": "Liberia"
    },
    {
        "code2": "LY",
        "name_fr": "Libye"
    },
    {
        "code2": "LI",
        "name_fr": "Liechtenstein"
    },
    {
        "code2": "LT",
        "name_fr": "Lituanie"
    },
    {
        "code2": "LU",
        "name_fr": "Luxembourg"
    },
    {
        "code2": "MK",
        "name_fr": "Macédoine du Nord"
    },
    {
        "code2": "MG",
        "name_fr": "Madagascar"
    },
    {
        "code2": "MY",
        "name_fr": "Malaisie"
    },
    {
        "code2": "MW",
        "name_fr": "Malawi"
    },
    {
        "code2": "MV",
        "name_fr": "Maldives"
    },
    {
        "code2": "ML",
        "name_fr": "Mali"
    },
    {
        "code2": "MT",
        "name_fr": "Malte"
    },
    {
        "code2": "MA",
        "name_fr": "Maroc"
    },
    {
        "code2": "MH",
        "name_fr": "Îles Marshall"
    },
    {
        "code2": "MU",
        "name_fr": "Maurice"
    },
    {
        "code2": "MR",
        "name_fr": "Mauritanie"
    },
    {
        "code2": "MX",
        "name_fr": "Mexique"
    },
    {
        "code2": "FM",
        "name_fr": "États fédérés de Micronésie"
    },
    {
        "code2": "MD",
        "name_fr": "Moldavie"
    },
    {
        "code2": "MC",
        "name_fr": "Monaco"
    },
    {
        "code2": "MN",
        "name_fr": "Mongolie"
    },
    {
        "code2": "ME",
        "name_fr": "Monténégro"
    },
    {
        "code2": "MZ",
        "name_fr": "Mozambique"
    },
    {
        "code2": "MM",
        "name_fr": "Birmanie"
    },
    {
        "code2": "NA",
        "name_fr": "Namibie"
    },
    {
        "code2": "NR",
        "name_fr": "Nauru"
    },
    {
        "code2": "NP",
        "name_fr": "Népal"
    },
    {
        "code2": "NI",
        "name_fr": "Nicaragua"
    },
    {
        "code2": "NE",
        "name_fr": "Niger"
    },
    {
        "code2": "NG",
        "name_fr": "Nigeria"
    },
    {
        "code2": "NO",
        "name_fr": "Norvège"
    },
    {
        "code2": "NZ",
        "name_fr": "Nouvelle-Zélande"
    },
    {
        "code2": "OM",
        "name_fr": "Oman"
    },
    {
        "code2": "UG",
        "name_fr": "Ouganda"
    },
    {
        "code2": "UZ",
        "name_fr": "Ouzbékistan"
    },
    {
        "code2": "PK",
        "name_fr": "Pakistan"
    },
    {
        "code2": "PW",
        "name_fr": "Palaos"
    },
    {
        "code2": "PA",
        "name_fr": "Panama"
    },
    {
        "code2": "PG",
        "name_fr": "Papouasie-Nouvelle-Guinée"
    },
    {
        "code2": "PY",
        "name_fr": "Paraguay"
    },
    {
        "code2": "NL",
        "name_fr": "Pays-Bas"
    },
    {
        "code2": "PE",
        "name_fr": "Pérou"
    },
    {
        "code2": "PH",
        "name_fr": "Philippines"
    },
    {
        "code2": "PL",
        "name_fr": "Pologne"
    },
    {
        "code2": "PT",
        "name_fr": "Portugal"
    },
    {
        "code2": "QA",
        "name_fr": "Qatar"
    },
    {
        "code2": "RO",
        "name_fr": "Roumanie"
    },
    {
        "code2": "GB",
        "name_fr": "Royaume-Uni"
    },
    {
        "code2": "RU",
        "name_fr": "Russie"
    },
    {
        "code2": "RW",
        "name_fr": "Rwanda"
    },
    {
        "code2": "KN",
        "name_fr": "Saint-Christophe-et-Niévès"
    },
    {
        "code2": "SM",
        "name_fr": "Saint-Marin"
    },
    {
        "code2": "VA",
        "name_fr": "Saint-Siège"
    },
    {
        "code2": "VC",
        "name_fr": "Saint-Vincent-et-les-Grenadines"
    },
    {
        "code2": "LC",
        "name_fr": "Sainte-Lucie"
    },
    {
        "code2": "SB",
        "name_fr": "Îles Salomon"
    },
    {
        "code2": "SV",
        "name_fr": "Salvador"
    },
    {
        "code2": "WS",
        "name_fr": "Samoa"
    },
    {
        "code2": "ST",
        "name_fr": "Sao Tomé-et-Principe"
    },
    {
        "code2": "SN",
        "name_fr": "Sénégal"
    },
    {
        "code2": "RS",
        "name_fr": "Serbie"
    },
    {
        "code2": "SC",
        "name_fr": "Seychelles"
    },
    {
        "code2": "SL",
        "name_fr": "Sierra Leone"
    },
    {
        "code2": "SG",
        "name_fr": "Singapour"
    },
    {
        "code2": "SK",
        "name_fr": "Slovaquie"
    },
    {
        "code2": "SI",
        "name_fr": "Slovénie"
    },
    {
        "code2": "SO",
        "name_fr": "Somalie"
    },
    {
        "code2": "SD",
        "name_fr": "Soudan"
    },
    {
        "code2": "SS",
        "name_fr": "Soudan du Sud"
    },
    {
        "code2": "LK",
        "name_fr": "Sri Lanka"
    },
    {
        "code2": "SE",
        "name_fr": "Suède"
    },
    {
        "code2": "CH",
        "name_fr": "Suisse"
    },
    {
        "code2": "SR",
        "name_fr": "Suriname"
    },
    {
        "code2": "SZ",
        "name_fr": "Eswatini"
    },
    {
        "code2": "SY",
        "name_fr": "Syrie"
    },
    {
        "code2": "TJ",
        "name_fr": "Tadjikistan"
    },
    {
        "code2": "TZ",
        "name_fr": "Tanzanie"
    },
    {
        "code2": "TD",
        "name_fr": "Tchad"
    },
    {
        "code2": "CZ",
        "name_fr": "Tchéquie"
    },
    {
        "code2": "TH",
        "name_fr": "Thaïlande"
    },
    {
        "code2": "TL",
        "name_fr": "Timor oriental"
    },
    {
        "code2": "TG",
        "name_fr": "Togo"
    },
    {
        "code2": "TO",
        "name_fr": "Tonga"
    },
    {
        "code2": "TT",
        "name_fr": "Trinité-et-Tobago"
    },
    {
        "code2": "TN",
        "name_fr": "Tunisie"
    },
    {
        "code2": "TM",
        "name_fr": "Turkménistan"
    },
    {
        "code2": "TR",
        "name_fr": "Turquie"
    },
    {
        "code2": "TV",
        "name_fr": "Tuvalu"
    },
    {
        "code2": "UA",
        "name_fr": "Ukraine"
    },
    {
        "code2": "UY",
        "name_fr": "Uruguay"
    },
    {
        "code2": "VU",
        "name_fr": "Vanuatu"
    },
    {
        "code2": "VE",
        "name_fr": "Venezuela"
    },
    {
        "code2": "VN",
        "name_fr": "Viêt Nam"
    },
    {
        "code2": "YE",
        "name_fr": "Yémen"
    },
    {
        "code2": "ZM",
        "name_fr": "Zambie"
    },
    {
        "code2": "ZW",
        "name_fr": "Zimbabwe"
    }
]
// Additionnal country
countryArrayEN.push({
    name: "Puerto Rico",
    code2: "PR",
    code3: "PRI"
})
countryArrayFR.push({
    name_fr: "Porto Rico",
    code2: "PR"
})
countryArrayEN.push({
    name: "Bermuda",
    code2: "BM",
    code3: "BMU"
})
countryArrayFR.push({
    name_fr: "Bermudes",
    code2: "BM"
})
countryArrayEN.push({
    name: "Virgin Islands (US)",
    code2: "VI",
    code3: "VIR"
})
countryArrayFR.push({
    name_fr: "Îles Vierges des États-Unis",
    code2: "VI"
})
countryArrayEN.push({
    name: "Bonaire, Sint Eustatius and Saba",
    code2: "BQ",
    code3: "BES"
})
countryArrayFR.push({
    name_fr: "Bonaire, Saint-Eustache et Saba",
    code2: "BQ"
})
countryArrayEN.push({
    name: "Hong Kong S.A.R.",
    code2: "HK",
    code3: "HKG"
})
countryArrayFR.push({
    name_fr: "R.A.S. chinoise de Hong Kong",
    code2: "HK"
})
countryArrayEN.push({
    name: "Kosovo",
    code2: "XK",
    code3: "XKX"
})
countryArrayFR.push({
    name_fr: "Kosovo",
    code2: "XK"
})
countryArrayEN.push({
    name: "Swaziland",
    code2: "SZ",
    code3: "SWZ"
})
countryArrayFR.push({
    name_fr: "Swaziland",
    code2: "SZ"
})
countryArrayEN.push({
    name: "Taiwan",
    code2: "TW",
    code3: "TWN"
})
countryArrayFR.push({
    name_fr: "Taïwan",
    code2: "TW"
})
countryArrayEN.push({
    name: "United States Minor Outlying Islands",
    code2: "US",
    code3: "USA"
})

export const countryENDict = countryArrayEN.reduce((acc, country) => {
    acc[country.code2] = country
    return acc
}, {} as { [key: string]: any })
export const countryFRDict = countryArrayFR.reduce((acc, country) => {
    acc[country.code2] = country
    return acc
}, {} as { [key: string]: any })
export const countryENByName = countryArrayEN.reduce((acc, country) => {
    acc[country.name.toLowerCase()] = country
    return acc
}, {} as { [key: string]: any })

export const countryData = countryArrayEN.map((country) => {
    return {
        ...country,
        ...countryFRDict[country.code2]
    }
})

console.log(countryENDict)
console.log(countryFRDict)